from Servidor.BDoperations import *
import _thread
    ...
    while True :
        ...
        #Funçoes de POST
            ...
            if len(mensage) == 1 :
                ...
                con.send("Send email, name and passwor".encode())
                data = con.recv(4096).decode()
                data = list(data.split('/'))
                con.send(addUser(data).encode())
            elif mensage[1] == 'EMAIL':
                con.send("Send token, mensage and target".encode())
                data = con.recv(4096).decode()
                data = list(data.split("/"))
            ...
            con.send("Mensagem Invalida".encode())
    #Fim da conexão
    con.close()
